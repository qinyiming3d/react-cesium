...
const lineRender = (viewer, data) => {
    const dataSource = new CustomDataSource('line');
    const sampleRate = Math.max(1, Math.floor(data.length / 10000)); // 采样率
    let sampledData = data.filter((_, index) => index % sampleRate === 0);

    console.log('采样后数据点数:', sampledData.length);
    sampledData = sampledData.filter(item => item[2]); // 过滤掉无效数据
    console.log('去除无效数据后的数据点数:', sampledData.length);

    // 生成等值线
    const contours = generateContours(sampledData);

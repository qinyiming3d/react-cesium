...
out vec2 textureCoordinate; // 输出到片段着色器
...
vec4 calculateProjectedCoordinate(vec3 lonLatLev) {
    ...
    vec4 projectedCoordinate = czm_modelViewProjection * vec4(particlePosition, 1.0); // 模型坐标到裁剪坐标，cesium是用笛卡尔坐标系作为模型坐标的

...
uniform float speedFactor; // 自定义粒子速度
...
void main() {
    ...
    vec3 lonLatLev = texture(currentParticlesPosition, v_textureCoordinates).rgb; // 当前位置经纬度。r通道x坐标（经度）  g通道y坐标（纬度）  b通道z坐标（高度） a通道为0
    vec3 speed = calculateSpeedByRungeKutta2(lonLatLev); // 二阶Runge-Kutta 方法计算粒子速度
    vec3 speedInLonLat = convertSpeedUnitToLonLat(lonLatLev, speed); // 单位转换，将风速从米/秒转换为经纬度单位

    vec4 particleSpeed = vec4(speedInLonLat, calculateWindNorm(speed / speedScaleFactor)); // 这里要除以速度的缩放因子，拿到真实的速度跟原始风速做对比（计算速度的适合乘了缩放因子）
